...
import {
    ...
    Briefcase, BookOpen, AlertTriangle, Shield, Megaphone, Check
} from 'lucide-react';
            ...
            <div className="p-6 md:p-10 grid grid-cols-1 xl:grid-cols-12 gap-8 md:gap-12 overflow-y-auto custom-scrollbar flex-1">
                ...
                <div className="xl:col-span-8 space-y-8 md:space-y-12">
                    <div className="bg-slate-50 p-6 md:p-12 rounded-[3rem] border border-slate-100 shadow-inner space-y-8 md:space-y-12">
                        ...
                        {/* --- NEW: AUDIENCE & VISIBILITY --- */}
                        <div className="pt-8 md:pt-12 border-t border-slate-200 space-y-6 md:space-y-8 animate-fade-in">
                            <div className="flex items-center gap-3">
                                <label className="text-[11px] font-black text-indigo-500 uppercase tracking-widest">مجال الرؤية والجمهور (Audience Scope)</label>
                                <span className="bg-indigo-50 text-indigo-500 px-2 py-0.5 rounded text-[8px] font-bold">من سيصلهم هذا الإعلان؟</span>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                                {/* OPTION 1: PRIVACY TOGGLE */}
                                <div className={`p-6 rounded-[2rem] border-2 transition-all cursor-pointer flex items-center justify-between ${data.audience?.isPrivate ? 'bg-indigo-50 border-indigo-500 shadow-lg' : 'bg-white border-slate-100'}`}
                                    onClick={() => updateData({ audience: { ...data.audience, isPrivate: !data.audience?.isPrivate } })}
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${data.audience?.isPrivate ? 'bg-indigo-500 text-white' : 'bg-slate-100 text-slate-400'}`}>
                                            {data.audience?.isPrivate ? <Shield size={24} /> : <Megaphone size={24} />}
                                        </div>
                                        <div>
                                            <h4 className={`text-sm font-black ${data.audience?.isPrivate ? 'text-indigo-900' : 'text-slate-600'}`}>
                                                {data.audience?.isPrivate ? 'حدث خاص (Private)' : 'حدث عام (Public)'}
                                            </h4>
                                            <p className="text-[10px] font-bold text-slate-400 mt-1">
                                                {data.audience?.isPrivate ? 'يظهر فقط للمنظم والمشاركين' : 'يظهر لجميع المعلمين في الرزنامة'}
                                            </p>
                                        </div>
                                    </div>
                                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${data.audience?.isPrivate ? 'border-indigo-500 bg-indigo-500' : 'border-slate-200'}`}>
                                        {data.audience?.isPrivate && <Check size={14} className="text-white" />}
                                    </div>
                                </div>

                                {/* OPTION 2: TARGET ROLES (If Scope is Public) */}
                                {!data.audience?.isPrivate && (
                                    <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-3">
                                        <label className="text-[10px] font-bold text-slate-400 flex items-center gap-2"><Users size={12} /> الفئات المستهدفة</label>
                                        <div className="flex flex-wrap gap-2">
                                            {[
                                                { id: 'teachers', label: 'المعلمون' },
                                                { id: 'educators', label: 'المربون' },
                                                { id: 'coordinators', label: 'المركزون' },
                                                { id: 'management', label: 'الإدارة' }
                                            ].map(role => {
                                                const isSelected = data.audience?.roles?.includes(role.id);
                                                return (
                                                    <button
                                                        key={role.id}
                                                        onClick={() => {
                                                            const currentRoles = data.audience?.roles || [];
                                                            const newRoles = isSelected
                                                                ? currentRoles.filter(r => r !== role.id)
                                                                : [...currentRoles, role.id];
                                                            updateData({ audience: { ...data.audience, roles: newRoles } });
                                                        }}
                                                        className={`px-3 py-2 rounded-xl text-[10px] font-black border transition-all ${isSelected ? 'bg-slate-800 text-white border-slate-800' : 'bg-slate-50 text-slate-500 border-slate-200'}`}
                                                    >
                                                        {role.label}
                                                    </button>
                                                )
                            ...
                            {/* OPTION 3: DEPARTMENTS (If Scope is Public) */}
                            {!data.audience?.isPrivate && (
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold text-slate-400 flex items-center gap-2 px-2"><Layers size={12} /> استهداف طواقم محددة (اختياري)</label>
                                    <div className="flex flex-wrap gap-2 p-4 bg-white border border-slate-100 rounded-[2rem]">
                                        {allSubjects.map(subject => {
                                            const isSelected = data.audience?.departments?.includes(subject);
                                            return (
                                                <button
                                                    key={subject}
                                                    onClick={() => {
                                                        const currentDepts = data.audience?.departments || [];
                                                        const newDepts = isSelected
                                                            ? currentDepts.filter(d => d !== subject)
                                                            : [...currentDepts, subject];
                                                        updateData({ audience: { ...data.audience, departments: newDepts } });
                                                    }}
                                                    className={`px-3 py-1.5 rounded-lg text-[9px] font-bold border transition-all ${isSelected ? 'bg-indigo-100 text-indigo-700 border-indigo-200' : 'bg-white text-slate-500 border-slate-200 hover:border-indigo-200'}`}
                                                >
                                                    طاقم {subject}
                                                </button>
                                            );
