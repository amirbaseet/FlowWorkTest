...
import { UserCheck, Minus, Trash2, X, Briefcase, User, Edit, GraduationCap, Check, Shield, Zap, AlertTriangle, UserX } from 'lucide-react';
import { AbsenceRecord, SubstitutionLog, Employee, Lesson, ClassItem } from '@/types';
import { DAYS_AR } from '@/constants';
import { normalizeArabic, createModeOverlay, ModeOverlayResult } from '@/utils';
...
interface DailyAbsenceSubstitutionGridProps {
    ...
    onAssignSubstitute?: (absentTeacherId: number, period: number, substituteId: number) => void; // New prop for assignment
}
...
const DailyAbsenceSubstitutionGrid: React.FC<DailyAbsenceSubstitutionGridProps> = ({
    ...
    maxPeriod = 7,
    date,
    onCancelAbsence, onEditAbsence, onUnassign, events = [], activeExternalIds = [], onAssignSubstitute
}) => {
    ...
    const subColumns = useMemo(() => {
        ...
        const uniqueSubIds = Array.from(new Set(substitutions.map(s => s.substituteId)));
        ...
        return uniqueSubIds.map(subId => {
            ...
            for (let p = 1; p <= maxPeriod; p++) {
                ...
                const assignment = substitutions.find(s => s.substituteId === subId && s.period === p);
    ...
    const renderLessonCell = (teacherId: number, period: number, isAbsentColumn: boolean) => {
        if (isAbsentColumn) {
            ...
            const substituteId = assignment?.substituteId;
            ...
            return (
                ...
                >
                    ...
                    {/* Mode-Aware Candidate Suggestions */}
                    {isUncovered && (
                        <div className="absolute inset-0 bg-rose-50/90 flex items-center justify-center z-20 opacity-0 group-hover/cell:opacity-100 transition-opacity">
                            <div className="bg-white p-2 rounded-lg shadow-lg w-48 max-h-32 overflow-y-auto">
                                <div className="text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
                                    <Zap size={10} className="text-amber-500" />
                                    مرشحون محتملون:
                                </div>
                                {getTopCandidates(teacherId, period).slice(0, 3).map((candidate, idx) => (
                                    <div
                                        key={candidate.teacherId}
                                        className="text-[9px] p-1.5 rounded flex items-center justify-between hover:bg-slate-50 cursor-pointer"
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            handleCandidateSelect(teacherId, period, candidate.teacherId);
                                        }}
                                    >
                                        <span>{candidate.teacherName}</span>
                                        <span className="text-amber-600 font-black">{candidate.score}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            // Substitute Cell
            const assignment = substitutions.find(s => s.substituteId === teacherId && s.period === period);
    ...
    // Helper function to get top candidates for a given absent teacher and period
    const getTopCandidates = (absentTeacherId: number, period: number) => {
        // Find the absent lesson to determine class and subject
        const absentLesson = lessons.find(l =>
        ...
        // Determine active mode for the date and class
        const activeEvent = events?.find(e =>
            e.date === date &&
            e.appliesTo.classes.includes(absentLesson.classId) &&
            e.opContext?.isActive
        );

        let modeType: 'EXAM' | 'TRIP' | 'RAINY' | 'HOLIDAY' | 'SHORTAGE' = 'SHORTAGE'; // Default fallback
        let params = {};
        ...
        if (activeEvent) {
            switch (activeEvent.eventType) {
                case 'EXAM':
                    modeType = 'EXAM';
                    params = {
                        examSubject: absentLesson.subject,
                        examType: 'standard',
                        gradesIncluded: [classes.find(c => c.id === absentLesson.classId)?.gradeLevel.toString() || '']
                    };
                    break;
                case 'TRIP':
                    modeType = 'TRIP';
                    params = {
                        tripClasses: [absentLesson.classId],
                        studentDismissAfterPeriod: 6,
                        escortCount: 2
                    };
                    break;
                case 'ACTIVITY':
                    if (activeEvent.title.toLowerCase().includes('rainy') || activeEvent.title.toLowerCase().includes('weather')) {
                        modeType = 'RAINY';
                        params = {
                            mergedClassCount: 1,
                            gradesIncluded: [classes.find(c => c.id === absentLesson.classId)?.gradeLevel.toString() || '']
        ...
        // Create mode overlay to get candidates
        const overlayResult = createModeOverlay(
            modeType,
            params as any,
            date,
            employees,
            lessons,
            classes,
            { periodsPerDay: 7 } as any, // Simplified config
            substitutions,
            events || []
        );

        // Return formatted candidates
        return overlayResult.candidateRankings.map(c => ({
            ...c,
    ...
    const handleCandidateSelect = (absentTeacherId: number, period: number, substituteId: number) => {
        // This function would be implemented by the parent component if needed
        onAssignSubstitute(absentTeacherId, period, substituteId);
    };
