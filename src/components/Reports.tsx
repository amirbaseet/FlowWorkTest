...
import {
   ...
   CloudRain, Bus, Wallet, Coins, ClipboardList, FileCheck, X, School
} from 'lucide-react';
import {
...
const Reports: React.FC<ReportsProps> = ({
   ...
   const stats = useMemo(() => {
      ...
      const activeExternals = new Set(substitutionLogs.filter(s => s.date === todayStr && s.type === 'assign_external').map(s => s.substituteId)).size;
   ...
   const operationalData = useMemo(() => {
      ...
      filteredLogs.filter(l => l.type === 'assign_external').forEach(l => {
         if (!payrollMap.has(l.substituteId)) {
            payrollMap.set(l.substituteId, {
               name: l.substituteName,
               hours: 0,
               days: new Set()
            });
         }
         const entry = payrollMap.get(l.substituteId)!;
         entry.hours++;
      ...
      filteredLogs.forEach(l => {
         teacherLoad[l.substituteId] = (teacherLoad[l.substituteId] || 0) + 1;
      });
   ...
   const ReportHeader = () => {
      ...
      const schoolLogo = scheduleConfig.schoolInfo?.logo;

      return (
         <div className="flex flex-col md:flex-row justify-between items-center mb-8 pb-8 border-b-2 border-slate-100/50 print:mb-12 print:pb-12 print:border-slate-800">
            <div className="flex items-center gap-6">
               <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center p-2 shadow-inner print:shadow-none print:border print:border-slate-800">
                  {schoolLogo ? (
               ...
               <div className="text-right">
                  <h1 className="text-3xl font-black text-slate-900 tracking-tighter print:text-black">{schoolName}</h1>
                  <p className="text-xs font-bold text-indigo-600 uppercase tracking-widest mt-1 print:text-black opacity-80">Strategic Reporting Unit</p>
               </div>
            </div>
            <div className="mt-6 md:mt-0 text-left md:text-right font-mono text-[10px] text-slate-400 flex flex-col gap-1 print:text-black">
               <span className="font-bold">كود التقرير: #RF-${Date.now().toString().slice(-6)}</span>
               <span className="font-black">تاريخ الإصدار: {new Date().toLocaleString('ar-EG')}</span>
            </div>
         </div>
      );
   };
