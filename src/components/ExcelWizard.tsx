...
import { Employee, ClassItem, Lesson, ScheduleConfig, SharedLesson } from '@/types';
...
const ExcelWizard: React.FC<ExcelWizardProps> = ({ 
  ...
  // Shared lesson classification state
  const [sharedLessonsQueue, setSharedLessonsQueue] = useState<SharedLesson[]>([]);
  ...
  const handleProcessFiles = async () => {
    ...
    try {
      ...
      // Check if shared lessons detected
      if (result.sharedLessons && result.sharedLessons.length > 0) {
        setSharedLessonsQueue(result.sharedLessons);
        setShowClassifier(true);
        setPayload(result); // Store payload for later use
      } else {
        setPayload(result);
        setStep(2);
      }
  ...
  // Handle classifications from SharedLessonClassifier
  const handleClassifications = (classifiedData: any[]) => {
    setClassifications(classifiedData);
    setShowClassifier(false);
    
    if (payload) {
      const updatedPayload = applyClassificationsToPayload(payload, classifiedData);
  ...
  // Apply classifications to payload entries
  const applyClassificationsToPayload = (
    ...
    const updatedEntries = payload.entries.map(entry => {
      ...
      const role = subjectRoleMap.get(normalizedSubject) || 'primary';
    ...
    return {
      ...
      sharedLessons: [] // Clear queue
    };
  };

  const applyChanges = (overriddenPayload?: any) => {
    // Ensure we don't treat the MouseEvent as a payload
    const targetPayload = (overriddenPayload && 'entries' in overriddenPayload) ? overriddenPayload : payload;
    if (!targetPayload) return;

    if (targetPayload.sharedLessons.length > 0 && !showClassifier) {
        setShowClassifier(true);
        return;
    }
    ...
    if (scheduleConfig && setScheduleConfig && targetPayload.schoolStats.periodCount > 0) {
       // Only update if import has MORE periods, or if we want strict sync
       // Here we force sync to match imported structure
       if (targetPayload.schoolStats.periodCount !== scheduleConfig.periodsPerDay) {
           setScheduleConfig(prev => ({
               ...prev,
               periodsPerDay: targetPayload.schoolStats.periodCount
           }));
           addToast(`تم تحديث هيكلية اليوم إلى ${targetPayload.schoolStats.periodCount} حصص`, "info");
       }
    }

    // 3. Merge Employees
    const nextEmployees = [...currentEmployees];
    targetPayload.teachers.forEach((wt, idx) => {
    ...
    targetPayload.classes.forEach(wc => {
    ...
    const importedClassKeys = new Set(targetPayload.classes.map(c => c.key));
    ...
    targetPayload.entries.forEach(entry => {
      ...
      if ((cls || isStayOrInd) && teacher) {
        nextLessons.push({
          ...
          teacherRole: entry.teacherRole || 'primary'
        });
      }
    });
  ...
  const handleClassifyAndApply = (classifications: any[]) => {
    // This is now replaced by handleClassifications and applyClassificationsToPayload
    handleClassifications(classifications);
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[9999] flex items-center justify-center p-4 animate-fade-in" dir="rtl">
      {showClassifier && sharedLessonsQueue.length > 0 && (
        <SharedLessonClassifier 
            sharedLessons={sharedLessonsQueue}
            onClassify={handleClassifications}
            onSkip={() => { setShowClassifier(false); setStep(2); }}
        />
      )}
