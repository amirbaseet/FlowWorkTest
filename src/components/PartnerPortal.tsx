...
import { Employee, ClassItem, CalendarEvent, ScheduleConfig } from '@/types';
...
const PartnerPortal: React.FC<PartnerPortalProps> = ({ employees, classes, setEvents, onBack, currentUser, scheduleConfig }) => {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 animate-fade-in font-sans" dir="rtl">
      <div className="fixed top-10 flex items-center gap-4">
        <div className="bg-indigo-600 p-3 rounded-2xl shadow-xl shadow-indigo-200">
          {schoolLogo ? (
            <img src={schoolLogo} alt="School Logo" className="w-6 h-6 object-contain filter brightness-0 invert" />
          ) : (
            <School className="text-white" size={24} />
          )}
        </div>
        <h1 className="text-2xl font-black text-slate-900 tracking-tighter">
          نظام {schoolName} <span className="text-indigo-600">الذكي</span>
        </h1>
      </div>
