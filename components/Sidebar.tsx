...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const menuItems: { id: ViewState; label: string; icon: any; color?: string; adminOnly?: boolean }[] = [
    { id: 'dashboard', label: 'لوحة التحكم', icon: LayoutDashboard },
    { id: 'bulletin-board', label: 'لوحة الإعلانات', icon: Tv, color: 'text-emerald-400' },
    { id: 'employees', label: 'طاقم العمل', icon: Users, adminOnly: true },
    { id: 'calendar', label: 'الرزنامة المدرسية', icon: CalendarDays },
    { id: 'schedule', label: 'الجدول الدراسي', icon: Calendar },
    { id: 'substitutions', label: 'الإشغال الذكي', icon: ShieldAlert },
    { id: 'reports', label: 'التقارير الإدارية', icon: FileBarChart2, adminOnly: true },
    { id: 'partner-portal', label: 'بوابة الشركاء', icon: Share2, color: 'text-violet-400' },
  ...
  return (
    <>
      ...
      {/* Sidebar Container - Added h-full explicitly for static positioning */}
      <div className={`
        fixed lg:static inset-y-0 right-0 z-50 w-64 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out flex flex-col h-full
