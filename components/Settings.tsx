...
import { 
  ...
  CloudRain, Bus, FileText, EyeOff, ShieldQuestion, UserX, Cpu, Radio, Terminal, BarChart3,
...
interface SettingsProps {
  ...
  validateDeleteClass: (classId: string) => { canDelete: boolean; reason?: string };
  validateDeleteRole: (roleId: string) => { canDelete: boolean; reason?: string };
  setEmployees?: React.Dispatch<React.SetStateAction<Employee[]>>;
  setLessons?: React.Dispatch<React.SetStateAction<Lesson[]>>;
}

const PERMISSION_OPTIONS = [
  { id: 'view_all', label: 'عرض الجداول الشاملة' },
  { id: 'edit_absences', label: 'توثيق الغياب' },
  { id: 'manage_staff', label: 'إدارة شؤون الموظفين' },
  { id: 'override_engine', label: 'تجاوز قرارات المحرك' },
  { id: 'system_admin', label: 'إدارة إعدادات النظام' }
];
...
const Settings: React.FC<SettingsProps> = ({ 
    scheduleConfig, setScheduleConfig, roles, setRoles, classes, setClasses,
    engineContext, setEngineContext, validateDeleteClass, validateDeleteRole,
    setEmployees, setLessons
}) => {
  ...
  const calculatedTimeline = useMemo(() => {
    const timeline: { type: 'period' | 'break'; id: number; start: string; end: string; duration: number; bType?: BreakType }[] = [];
  ...
  const handleAddRole = () => {
    const newRole: Role = { id: `role_${Date.now()}`, label: 'منصب جديد', defaultHours: 36, permissions: ['view_all'], workloadDetails: { actual: 26, individual: 5, stay: 5 } };
                 ...
                 {calculatedTimeline.map((slot, idx) => (
                    <div key={`${slot.type}-${slot.id}`} className={`flex items-center gap-6 transition-all group ${slot.type === 'break' && slot.bType === 'none' ? 'opacity-30 hover:opacity-100' : ''}`}>
                       ...
                       <div className="relative flex flex-col items-center">
                          <div className={`w-4 h-4 rounded-full border-2 z-10 ${slot.type === 'period' ? 'bg-indigo-500 border-indigo-300 shadow-[0_0_10px_rgba(99,102,241,0.5)]' : slot.bType !== 'none' ? 'bg-amber-500 border-amber-300' : 'bg-slate-700 border-slate-600'}`}></div>
                          {idx !== calculatedTimeline.length - 1 && <div className="w-0.5 h-16 bg-white/10 absolute top-4"></div>}
                       </div>

                       {/* Card Config */}
                       <div className={`flex-1 p-4 rounded-[2rem] border transition-all flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0 ${slot.type === 'period' ? 'bg-white/5 border-white/10 hover:bg-white/10' : slot.bType !== 'none' ? 'bg-amber-500/10 border-amber-500/30' : 'bg-transparent border-transparent hover:bg-white/5'}`}>
                          <div className="flex items-center gap-4">
                             <div className={`p-3 rounded-2xl ${slot.type === 'period' ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400'}`}>
                                {slot.type === 'period' ? <span className="font-black text-sm">{slot.id}</span> : <Coffee size={16}/>}
                             </div>
                             <div>
                                <span className={`text-sm font-black block ${slot.type === 'period' ? 'text-white' : 'text-amber-400'}`}>
                                   {slot.type === 'period' ? `الحصة ${slot.id}` : slot.bType === 'long' ? 'استراحة رئيسية' : slot.bType === 'short' ? 'استراحة قصيرة' : 'فاصل زمني'}
                                </span>
