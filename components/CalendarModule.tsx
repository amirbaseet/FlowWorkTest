...
import { resolveDay, timeToMins, toLocalISOString } from '../utils';
...
const CalendarModule: React.FC<CalendarModuleProps> = ({
  ...
  comments, employees, classes, lessons, absences, setAbsences, 
  substitutionLogs, setHolidays, setOverrides, setEvents, setTasks, 
  ...
  const externalForce = useMemo(() => {
      ...
      return externals.map(ext => {
          const load = daySubs.filter(s => s.substituteTeacherId === ext.id).length;
    ...
    return (
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col h-full animate-slide-up">
         ...
         <div className="grid grid-cols-[60px_repeat(7,1fr)] bg-slate-50 border-b border-slate-200">
            ...
            {weekDays.map((d, i) => {
                const dateStr = toLocalISOString(d); // FIX: Use local date
                const isToday = toLocalISOString(new Date()) === dateStr;
                return (
                    <div key={i} className={`p-4 text-center border-l border-slate-100 last:border-0 ${isToday ? 'bg-indigo-50' : ''}`}>
                        <span className={`block text-xs font-black ${isToday ? 'text-indigo-600' : 'text-slate-800'}`}>{DAYS_AR[d.getDay()]}</span>
                        <span className="block text-[10px] font-bold text-slate-400 mt-1">{d.getDate()} {MONTHS_AR[d.getMonth()]}</span>
                    </div>
                );
            })}
         </div>
                 ...
                 <div key={p} className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-slate-100 min-h-[100px]">
                     ...
                     {weekDays.map((d, i) => {
                         ...
                         return (
                             <div 
                                key={`${dateStr}-${p}`} 
                                className="border-l border-slate-100 p-2 relative group hover:bg-slate-50/50 transition-colors"
                                onClick={() => onOpenRequestForm({ date: dateStr, period: p })}
                             >
                ...
                {days.map((d, i) => {
                    ...
                    return (
                        <div 
                            ...
                            className={`rounded-3xl p-3 border transition-all cursor-pointer flex flex-col ${isToday ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-slate-100 hover:border-indigo-200 hover:shadow-md'}`}
                        >
                      ...
                      <div key={m} className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-all">
                          ...
                          <div className="grid grid-cols-7 gap-1 text-center">
                              ...
                              {grid.map((d, idx) => {
                                  ...
                                  const dateStr = toLocalISOString(new Date(year, m, d)); // FIX: Use local date
                                  const hasEvent = events.some(e => e.date === dateStr);
                                  return (
                                      <div key={idx} className={`aspect-square flex items-center justify-center rounded-lg text-[9px] font-bold ${hasEvent ? 'bg-indigo-100 text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`}>
                                          {d}
                                      </div>
                                  );
                              })}
                  ...
                  return (
                      <div key={m} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                          <div className="bg-slate-50 p-4 border-b border-slate-100 flex items-center gap-3">
                              <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center font-black text-slate-800 shadow-sm">{m + 1}</div>
                              <h4 className="font-black text-lg text-slate-800">{MONTHS_AR[m]} {year}</h4>
                          </div>
                          ...
                          <div className="p-4 flex gap-3 overflow-x-auto custom-scrollbar">
                              {days.map(d => {
                                  ...
                                  return (
                                      <div 
                                        key={d} 
                                        className={`flex flex-col items-center gap-2 p-3 rounded-2xl border min-w-[100px] hover:shadow-md transition-all group cursor-pointer ${
