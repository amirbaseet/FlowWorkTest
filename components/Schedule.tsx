...
import { Lesson, ScheduleConfig, Employee, ClassItem, LessonType, AcademicYear, DayPattern, CalendarHoliday, DayOverride, CalendarEvent, PeriodSlot, EngineContext, ResolvedDay, ScheduleFilter, ModeConfig, SubstitutionLog, AbsenceRecord } from '../types';
...
const Schedule: React.FC<ScheduleProps> = ({ 
  ...
  const visibleDays = useMemo(() => {
    ...
    if (hideHolidays) {
        sorted = sorted.filter(day => !scheduleConfig.holidays.includes(day));
    }
  ...
  const resolveLayeredContent = (day: string, period: number) => {
      ...
      if (viewMode === 'teacher') {
          ...
          const myCoverage = substitutionLogs.find(s => 
              s.date === dateStr && 
              s.period === period && 
              s.substituteTeacherId === Number(selectedEntityId)
          );
          ...
          finalLessons = finalLessons.map(l => {
              ...
              if (teacherAbsence) {
                  ...
                  if (subLog) {
                      return { ...l, teacherId: subLog.substituteTeacherId, type: 'SUBSTITUTED' };
                  } else {
                                 ...
                                 {isHoliday ? (
                                    ...
                                    <div className="h-full flex flex-col gap-1">
                                       ...
                                       {baseLessons.map((lesson, lIdx) => {
                                          ...
                                          if (lesson.type === 'stay') cardStyle = "bg-amber-50/50 border-amber-100 text-amber-700";
                                          if (lesson.type === 'individual') cardStyle = "bg-emerald-50/50 border-emerald-100 text-emerald-700";

                                          return (
                                             <div key={lIdx} className={`p-2 rounded-xl border ${cardStyle} shadow-sm text-right relative group overflow-hidden`}>
