...
import { Lesson, Employee, ClassItem, SubstitutionLog, AbsenceRecord } from '../types';
...
const AiAssistant: React.FC<AiAssistantProps> = ({ lessons, employees, classes, substitutionLogs = [], absences = [] }) => {
  const { addToast } = useToast();
  const [messages, setMessages] = useState<any[]>([
    { id: 'welcome', role: 'model', content: 'أهلاً بك في العقل المركزي لـ "مدرستي". أنا مطلع على حالة الطاقم والرزنامة اليومية الآن. كيف يمكنني دعم اتخاذ القرار اليوم؟' }
  ]);
  ...
  const generateResponse = async (prompt: string) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: prompt,
        config: {
          systemInstruction: `You are the Brain of "Madrasati" School System. ${fieldAwareness} Answer with deep logic, considering the current operational status. Always prioritize educational stability.`,
        },
      });
